            ...
            {products.length > 0 ? (
                products.map((product) => (
                    <div key={product.id}>
                        ...
                        <p className="text-black font-bold mt-4 ">{product.title}</p>
                        <p className="text-[#0A0A0A] mt-[8.6px]">{product.description}</p>
                        <p className="text-black font-bold mt-[10.85px]">${product.price}</p>
                        <div className="flex items-center mb-4">
                            {[...Array(5)].map((_, index) => (
                                <FaStar key={index} color={index < filledStars ? '#ffc107' : '#c4c4c4'} />

                            ))} <div className=' ml-5 text-gray-500'>({product.stock})</div>
                        </div>
                    </div>
                ))
            ) : (
